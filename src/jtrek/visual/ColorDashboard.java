...
public class ColorDashboard extends Dashboard {
	...
	public void update(Graphics g) {
		...
		if (data.me.speed >= data.me.ship.maxspeed - 2)
			...
			color = Color.white;
		drarBar(og, "Sp", 90, 2, data.me.speed, cur_max, data.me.ship.maxspeed, 3, color);
		...
		if (value <= 50)
			...
			color = Color.white;
		drarBar(og, "Sh", 90, 14, data.me.ship.maxshield - data.me.shield, data.me.ship.maxshield, data.me.ship.maxshield, 3, color);
		
		// draw damage
		value = (100 * data.me.damage) / data.me.ship.maxdamage;
		if (value <= 10)
			color = Color.white;
		else if (value > 50)
			color = Color.red;
			...
			color = Color.yellow;
		drarBar(og, "Da", 90, 26, data.me.damage, data.me.ship.maxdamage, data.me.ship.maxdamage, 3, color);
		...
		if (value <= 3)
			...
			color = Color.yellow;
		drarBar(og, "Ar", 218, 2, data.me.armies, cur_max, data.me.ship.maxarmies, 3, color);
		...
		if (value > 50)
			...
			color = Color.yellow;
		drarBar(og, "Wt", 218, 14, data.me.wtemp / 10, data.me.ship.maxwpntemp / 10, data.me.ship.maxwpntemp / 10, 3 ,color);
		...
		if (value <= 25)
			...
			color = Color.red;
		drarBar(og, "Et", 218, 26, data.me.etemp / 10, data.me.ship.maxegntemp / 10, data.me.ship.maxegntemp / 10, 3 ,color);
		...
		if (value <= 50)
			...
			color = Color.yellow;
		drarBar(og, "Fu", 346, 2, data.me.fuel, data.me.ship.maxfuel, data.me.ship.maxfuel, 5, color);
	...
	void drarBar(Graphics g, String header, int x, int y, int value, int tmpmax, int max, int digits, Color color)  {
		char[] buffer = null;
		switch (digits) {
		case 3:
			buffer = new char[11];
